...
| --- | --- | --- |
| Model or application to action input | Model output can be malformed, over-broad, or adversarial. | **Implemented in the current code path:** canonical per-action parameter validation, a bounded/cycle-rejecting action-metadata normalizer, minimum risk floors, bounded non-reflective diagnostics for unsupported outer action fields and values, and per-action policy evaluation. Exported JSON Schemas can assist structured generation and offline shape checks, but are not an authorization or security control. The metadata bound is not a general plan-size or key-count quota, and structural validity does not establish safe semantic intent. |
