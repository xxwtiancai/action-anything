...
raw-identity-aware normalizer with a 64-container-per-nested-path boundary, direct result
...
The hypothesis is falsified if a 64-container nested path is rejected, a 65th
container on one path is accepted, a cycle reaches a trace, or invalid
executor output is reported as a successful action.

## Scope

- Apply the existing per-path 64-container raw-identity cycle contract to the
  required root `ActionResult.output` mapping, counting every
  mapping/list/tuple container on a nested path, including the root.
...
The per-path 64-container contract prevents unbounded recursive shape
processing but is not a size, width, string-length, or CPU limit. Custom
executors remain application-owned and can have independent side effects or
resource costs.
